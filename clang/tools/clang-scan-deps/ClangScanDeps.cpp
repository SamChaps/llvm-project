...
int main(int argc, const char **argv) {
  ...
  std::unique_ptr<DependencyScanningTool> WorkerTools;
  WorkerTools = llvm::make_unique<DependencyScanningTool>(*AdjustingCompilations);
  
  std::atomic<bool> HadErrors(false);
  HadErrors = WorkerTools->runOnFile(Inputs[0].first, Inputs[0].second) || HadErrors;
    ...
    WorkerThreads.emplace_back(
        [I, &Lock, &Index, &Inputs, &HadErrors, &WorkerTools]() {
          while (true) {
            ...
            // Run the tool on it.
            HadErrors = WorkerTools[I]->runOnFile(Input, CWD) || HadErrors;
          }
        });
  }
